...
import com.mmusic.player.ui.folders.FolderViewModel
...
fun FolderDetailsScreen(
    path: String,
    folderDetailsViewModel: FolderDetailsViewModel = hiltViewModel(),
    backPressed: () -> Unit,
) {
val context= LocalContext.current
    LaunchedEffect(key1 = path) {
        folderDetailsViewModel.fetchFolderSongs(path)
    }
    ...
    val artistsDetailsUiState by folderDetailsViewModel.folderDetailsUiStateState.collectAsState()
    ...
    Column(verticalArrangement = Arrangement.SpaceBetween, modifier = Modifier.fillMaxSize()) {
        ...
        folderDetailsViewModel.shuffleSongs(artistsDetailsUiState.songsList)
            }
        ...
        LazyColumn {
            ...
            itemsIndexed(artistsDetailsUiState.songsList) {index,song->
                ...
                SongItem(song = song){
                    folderDetailsViewModel.playSongs(artistsDetailsUiState.songsList,index)
                }
            }
        }
        ...
        if (showSortingSheet) {
            SortingBottomSheet(
                ...
                sortingApplied = {
                    folderDetailsViewModel.onSortClick(it)
                },
